...
import { evmProviderName, sponsorAddress } from "@pera/evm";
...
export async function statusRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/status", async () => {
    ...
    return {
      ...
      users,
      database: db,
      sponsor: { publicKey: sponsorPub, explorerUrl: stellarAccountUrl(sponsorPub), xlm: sponsor?.xlm ?? null, lowXlm: sponsor ? Number(sponsor.xlm) < 50 : null, evmAddress: sponsorAddress() ?? null },
      ...
      evm: { provider: evmProviderName(), gasSponsorship: evmProviderName() === "privy" ? `privy (${env.PRIVY_GAS_SPONSORSHIP})` : "sponsor EOA relay", network: "eip155:84532", usdc: BASE_SEPOLIA.usdc, messageTransmitterV2: BASE_SEPOLIA.messageTransmitterV2, cctpDomain: CCTP.baseSepoliaDomain },
