...
/** Stores an owner-approved ruleset. A ruleset that leaves the approval threshold out keeps the current one. */
async function applyRules(userId: string, rules: z.infer<typeof RulesBody>): Promise<void> {
  await upsertAgentRules(userId, { ...rules, approveAboveUsdc: rules.approveAboveUsdc === undefined ? ((await getAgentRules(userId)).approveAboveUsdc ?? null) : rules.approveAboveUsdc });
}
...
export async function agentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/agent/policy", async (req) => {
    ...
    if (rules) await applyRules(ctx.userId, rules);
  ...
  app.put("/agent/rules", async (req, reply) => {
    ...
    await applyRules(ctx.userId, RulesBody.parse(JSON.parse(ch.payload)));
