...
import { createSoftwarePasskey } from "@pera/passkey";
...
/** Rule changes are approved by the owner's passkey: sign the challenge with this device's software passkey. */
const setRules = async (rules: Record<string, unknown>) => {
  const deviceFile = path.join(path.dirname(sessionFile), "passkey.json");
  if (!existsSync(deviceFile)) throw new Error(`no device passkey at ${deviceFile}; rule changes need it`);
  const d = JSON.parse(readFileSync(deviceFile, "utf8")) as { rpId: string; origin: string; privateKeyPem: string; credentialId: string };
  const passkey = createSoftwarePasskey({ rpId: d.rpId, origin: d.origin, privateKeyPem: d.privateKeyPem, credentialId: d.credentialId });
  const { challenge } = await rest<{ challenge: string }>("POST", "/agent/rules/options", rules);
  return rest("PUT", "/agent/rules", { challenge, assertion: await passkey.assert(challenge) });
};
...
await setRules({ weeklyCapUsdc: rulesBefore.weeklyCapUsdc, maxPerCallUsdc: rulesBefore.maxPerCallUsdc, allowedNetworks: rulesBefore.allowedNetworks, approveAboveUsdc: "0.005" });
try {
  ...
  await setRules({ weeklyCapUsdc: rulesBefore.weeklyCapUsdc, maxPerCallUsdc: rulesBefore.maxPerCallUsdc, allowedNetworks: rulesBefore.allowedNetworks, approveAboveUsdc: rulesBefore.approveAboveUsdc ?? null });
}
