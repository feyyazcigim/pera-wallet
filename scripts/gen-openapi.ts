...
const paths = {
  ...
  "/agent/pay/jobs/{id}": { get: { summary: "Payment job started by POST /agent/pay?async=1 (Base payments bridge via CCTP first); ?wait=<s> long-polls up to 60 s. Failed jobs answer with the payment's error", security: bearer, parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }, { name: "wait", in: "query", schema: { type: "integer", minimum: 0, maximum: 60 } }], responses: { 200: obj("{ jobId, status: pending|paid|failed, url, steps[], result?, error? }"), 404: err } } },
